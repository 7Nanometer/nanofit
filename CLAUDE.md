...
- 力量档位推荐优先级（src/lib/kcal.ts 的 recommendMetLevel，取第一个命中的）：
  ①组间休息中位数<60秒→循环 ②含大重量复合动作→高强度 ③时长<30分或组数<8→低强度 ④中等
  ★ 组间休息用**中位数**不用平均数：中间接个电话就会把平均数彻底带偏。
...
- 结束训练时弹 MetPicker（src/components/MetPicker.tsx）让主人确认档位。
  ★ "自动推荐"和"记住上次选的"是这样共存的：第一次默认用推荐值，
    之后默认用上次选的，但"建议"那个小标签永远标在**本次推荐**的那一档上。
