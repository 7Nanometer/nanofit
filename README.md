...
三张都是**按手机尺寸直接截的**（375×667），没有美化、没有修图。
里面的训练记录是**示例数据** —— 不填点东西的话统计页那几张图是空的，
没法看出它长什么样。界面本身是什么样，图上就是什么样。
...
<p align="center">
  ...
  <br />
  <em>统计页 —— 顶上四个数：本周总容量、最近一次、本周练了多久、平均每次。<br />
  下面是每周总容量的柱状图；再往下还有训练时长趋势、单个动作的
  最大重量 / 总容量 / 估算 1RM 三条曲线、有氧的记录、体重体脂。</em>
</p>
...
- **组间休息计时器**：默认 90 秒可调，结束有三声提示音。
  **安卓 App 上到点还会发系统通知** —— 切到微信、锁屏了也一样叫得醒你
  （第一次用会问你要通知权限）。
  **每一条提醒都会响、会震**，切到别的 App、锁屏都一样。
  （这两件事在 2026-09-26 之前都是坏的：只有当天第一条会响会震，后面每一条都
  悄没声的；而且震动从来就没成功过。原因是插件建渠道时只开震动开关、
  没给震动节奏。见下面「几处看起来奇怪但故意这么写」里那两条。）
...
- **历史记录**：按日期倒序，可以只看某个动作的历史（对比自己三个月前的重量）。
  ...
  **一天练两次就是两条记录**，同一天里晚的排在上面（和整页"最近的在最上面"同一个方向）。
  （2026-09-26 之前是坏的：同一天练第二场会把第一场顶掉。）
...
| | 怎么定 | 值 |
|---|---|---|
| **力量训练** | 你结束训练时自己选一档，App 会先按数据推荐一个 | 低强度 3.0 / 中等 3.5 / **中高强度 5.0** / 高强度 6.0 / 循环训练 8.0 |
| **有氧** | 按动作类型，跑步另外按速度 | 椭圆机 5.0、骑行 7.0、划船机 7.0、爬楼机 9.0、跳绳 12.0…… |

推荐那一档的依据，**首选是你自己记的 RPE** —— 那是你亲口说的"这一组有多难"，
比什么推测都准。规则是：

1. **先把热身组摘出去。** RPE 不到 6 的组算热身，不参与下面的判断 ——
   热身不代表这次训练的强度，让它参与只会把结果往下拽。
2. 剩下那些组的 RPE **中位数**落在哪一段（面板上会把这个数直接写给你看）：
   - **9 分及以上** → 高强度 6.0
   - **7.5 到 9 分** → 中高强度 5.0
   - **不到 7.5 分** → 中等 3.5
3. 再叠一条：如果中位数 **8 分以上、而且每组之间隔不到 60 秒** → 循环训练 8.0。
   （这条的门槛比"高强度"那条低，是故意的：它判的是"喘不上气地连轴转"，
   和"单组很重"不是一回事。）
4. **整场都是热身**（比如热身完就走了）→ 低强度 3.0。

**没记 RPE 的时候**才退回按数据猜：每组之间隔多久、有没有练深蹲/硬拉/推胸这类
多关节大重量动作（杠铃和固定器械都算）、整场时长和组数。
（这条路上**猜不到"中高强度 5.0"** —— 没记 RPE 时本来就只是兜底，
那时候你自己知道这场练了什么，直接选就行。）
...
产物在 `android/app/build/outputs/apk/debug/app-debug.apk`，大约 5 MB。
第一次跑要 2 分钟左右，之后改代码再打会快很多（改完网页代码要**先**
`npm run build && npx cap sync android`，否则打进包里的是旧网页资源）。

> ✅ **当前状态**：最新一个包是 **2026-09-26**（`2026.09.26 · 52ce72c`）。
> 为了好找，另外复制了一份到仓库外面：
> `E:\Vibe Coding\Nanofit\安卓安装包\NanoFIT.apk`
>
> **真机验收的进度**（全仓库唯一一处"哪儿验过、哪儿没验过"的实话）：
> - **2026-09-23** —— 装到真机，确认可用
> - **2026-09-26 `a112b45`** —— 装到真机，两轮实测：切到音乐 App / 微信 /
>   bilibili 三种场景提醒都生效，锁屏也生效，**震动每次都有的**。
> - 它之后的**两个包都还没在真机上跑过**：`118ff04`（拆掉临时诊断、顺手删旧渠道）、
>   `52ce72c`（强度档位从四档变五档、判定时排除热身组）。
>   电脑上能验的都验了（见各次提交说明），但"装到手机上会不会出问题"
>   只有装上去才知道。
...
| `screens/TrainScreen.tsx` 落盘按 **id** 去重，不按 date | 原来是 `s.date !== session.date` —— 等于"这天有过记录就全删掉"，同一天练第二场会把第一场**顶掉**（而且从第一个能记训练的版本起就是这样，已经丢过的找不回来）。按 id 去重才只动"自己那一条"。★ 前提是 `completedSession()` **不重新生成 id**，改它时必须保住这条 |
| `lib/restnotify.ts` 通知 id 按"结束时刻"算 | 插件写死了 `setOnlyAlertOnce(true)`（`LocalNotificationManager.kt:199`）：**同一个 id 再次出现＝系统当成"更新"，不再响、不再震、不弹横幅**。原来固定用 1 号（本意是不攒一排），于是当天第一条提醒之后全哑。编号必须每次不同，但"同一次休息重复预约"要相同（否则会攒出一排）。⚠️ 上限是安卓的 32 位整数 **2,147,483,647**，模数取大了会溢出成负数 |
| `MainActivity.java` **自己建通知渠道** | 插件的 `createChannel` 只会调 `enableVibration(true)`，**从不设震动节奏**（整个插件里没有一处 `setVibrationPattern`）。而"开了震动开关、没给节奏"在真机上**就是不震**，系统设置里那个渠道**连「震动」都不显示**。真机实测 12 次全不震，自己建 + 给节奏之后**每次都震**。★ 所以这段 Java 不能删，删了震动立刻没了。`REST_CHANNEL_ID` 必须和 `restnotify.ts` 的 `CHANNEL_ID` 一字不差 |
| `lib/restnotify.ts` 删旧渠道前先确认新渠道在 | 换渠道编号时旧的 `rest-timer` 会一直挂在系统设置里（两条都叫「组间休息」，主人不知道看哪条）。**但只在新渠道确实存在时才删旧的** —— 反过来（新渠道没建起来、旧的又删了）的话，安卓 8 起"渠道不存在"的通知会被**直接丢掉**，提醒彻底失效 |
