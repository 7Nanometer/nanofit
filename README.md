...
- **组间休息计时器**：默认 90 秒可调，结束有三声提示音。
  **安卓 App 上到点还会发系统通知** —— 切到微信、锁屏了也一样叫得醒你
  （第一次用会问你要通知权限）。
  **每一条提醒都会响、会震**。（2026-09-26 之前是坏的：只有当天第一条会响会震，
  后面每一条都悄没声的。见下面「几处看起来奇怪但故意这么写」里通知 id 那一条。）
...
> ✅ **当前状态**：最新一个包是 **2026-09-26**（`2026.09.26 · 455f8b9`）。
> 为了好找，另外复制了一份到仓库外面：`E:\Vibe Coding\Nanofit\安卓安装包\NanoFIT.apk`
>
> ⚠️ **真机验收的进度**：只有 **2026-09-23** 那个包在真机上装过并且确认可用。
> 9-24、9-26 两个包**都还没在真手机上跑过** —— 电脑上能验的都验了
> （见各次提交说明），但"装到手机上会不会出问题"只有装上去才知道。
