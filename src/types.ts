...
export type WorkoutSession = {
  ...
  // 组间休息"结束的那一刻"（毫秒时间戳）。正在休息时才有。
  //
  // 【为什么要存进训练记录、而不是只放在页面内存里】
  // 底部四个 tab 是靠一个变量切换的，切走的时候训练页整个被卸载，
  // 页面内存里的东西全没了 —— 于是"休息中 1:23"会在你切到「历史」
  // 看一眼再切回来时凭空消失（数据没丢，就是倒计时没了）。
  // 存进正在进行的这次训练里，切回来就能接着走。
  //
  // 【存的是"结束时刻"而不是"还剩几秒"】
  // 和 RestTimer.tsx 顶部讲的是同一个道理：手机锁屏时定时器会被降频，
  // 按秒递减会走不准，记一个时间点则永远算得对。
  //
  // 训练结束时会把它抹掉，所以历史记录里不会留下这个字段。
  restEndsAt?: number
}
...
export type Settings = {
  ...
  // 上次"结束训练"时选的那一档强度。
  //
  // ★ 2026-09-24 起它**不再是下次的默认值**，这条注释以前写的是
  //   "下次默认就用它"，那是出过事的：默认值取"上次选的"，
  //   而"上次选的"一旦写进去就没有任何地方会作废它 ——
  //   一次选错，之后每场都跟着错。有人的 16 组大重量训练
  //   一直被按"低强度"算热量，差两倍半。
  //
  //   现在默认永远选**本次推荐**（见 lib/kcal.ts 的 recommendMetLevel）。
  //   这个值只用来在面板上给一个"沿用上次：XX"的快捷按钮 ——
  //   由用户自己点，不替他决定。
